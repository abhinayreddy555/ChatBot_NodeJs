var groceryOrder = function (builder) {
    ...
    var groceryMenuItems = ["Vegetables", "Groceries", "Drinks"];  
    this.welcomeDialog = {
        title: "Welcome",
        functions: [
            function (session) {
    ...
    this.menuDialog = {
        ...
        functions: [
            ...
            function (session, flags, results) {
                //if (results.response == "") {
                ...
                if (flags && flags.IsMoreOrder) {
                    builder.Prompts.choice(session, "Select more.. or type Check Out", groceryMenuItems, { listStyle: builder.ListStyle.button });
                }
                else {
                    session.userData.groceryItems = [];
                    builder.Prompts.choice(session, "Please select grocery item from list:", groceryMenuItems, { listStyle: builder.ListStyle.button });
                    session.userData.typeOfGrocery = results.response;
